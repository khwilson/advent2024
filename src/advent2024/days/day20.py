...
def twenty_ways(x: int, y: int, max_x: int, max_y: int) -> list[tuple[int, int, int]]:
            ...
            for (lx, ly), (rx, ry) in its.combinations(xx, 2):
                if maze[lx][ly] != "#" and maze[rx][ry] != "#":
                    ...
                    savings[(row, col)] = max(
                        savings[(row, col)],
                        the_dist
                        - (dist_from_start[(lx, ly)] + dist_to_end[(rx, ry)] + 2),
                    )
                    savings[(row, col)] = max(
                        savings[(row, col)],
                        the_dist
                        - (dist_to_end[(lx, ly)] + dist_from_start[(rx, ry)] + 2),
                    )
...
def part2(data_file: str | Path) -> int | str:
    ...
    for row in tqdm(range(1, height - 1)):
        for col in range(1, width - 1):
            ...
            for l, r, d in twenty_ways(row, col, height, width):
                if maze[l][r] != "#":
                    savings[(row, col, l, r)] = max(
                        savings[(row, col, l, r)],
                        the_dist
                        - (dist_from_start[(row, col)] + dist_to_end[(l, r)] + d),
                    )
